...
let string_to_range (s:string) = 
  ...
  | [a; b] -> (a, b)
  | _ | exception Failure _ -> raise (Malformed_Json "Invalid level range")
...
let encounters_of_json json = (json |> to_list |> List.map encounter_of_json)

let all_encounters_of_json json = json_list json encounters_of_json "encounters" |> Array.of_list

let sprites_of_json json = json_list json to_string "tile_sprites" |> Array.of_list
...
let list_to_tile_type encounters (s:string) =
  ...
  else Grass (String.sub s 1 (String.length s - 1) |> int_of_string |> Array.get encounters)
...
let codes_of_json json = 
  json |> to_string |> String.split_on_char ','

let init_map json = 
  let e = all_encounters_of_json json in
  let g = sprites_of_json json in
  json |> member "map" |> to_list |> List.map codes_of_json |> ll_to_matrix |> matrix_map (code_to_tile g e)
...
let encounter_creature e = 
  ...
  | None -> None
  | Some {name; level} -> Some (Creature.create_creature name level)e