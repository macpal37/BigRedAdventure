...
(* let load_maps _ = let t = Hashtbl.create 16 in Yojson.Basic.from_file
   "assets/maps/maps.json" |> to_list |> List.iter (fun j -> let file =
   j |> to_string in Hashtbl.add t file (Map.load_map file)); t *)
...
let load _ = assets_ref := Some { items = load_items () }
