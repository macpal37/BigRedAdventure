(** Representation of static pokemon data.
    ...
    This module represents the data stored in pokemon_list files,
    including the stats and moves. It handles loading of that data from
    JSON as well as querying the data. *)

type pokemon
(** The abstract type that represents a pokemon. All of a pokemon stats
    and features will be stored in this type. *)
...
(** Returns the pokemon's current and max hp as the tuple:
    (current_hp*max_hp) Used to display values*)

val get_stats : pokemon -> int list
(** Returns all of the pokemon's stats as a list The list can be ready
